...
def get_motif_format(content):
    ...
    if (
        re.match(r".*MEME version.+", content, re.DOTALL) is not None
    ):  # MOTIF\s.+letter-probability matrix.+[\d\.\s]+", content, re.MULTILINE) is not None:
        motif_format = "meme"
...
def get_TFs(path):
    ...
    TFs_tmp = list(
        map(
            lambda x: x.strip().replace(">", ""), filter(lambda x: x.startswith(">"), f)
        )
    )
...
path = (
    "/data/kopardevn/GitRepos/CCBR_tobias/resources/HOCOMOCOv11_HUMAN_core_motifs.txt"
)
TFs = get_TFs(path)
print(TFs)
